...
function load_pdb(fname::String, T=Float32)
    ...
    temp = convert(System{T}, orig_pdb)
    add_secondary_structures!(temp, fname)
end

function add_secondary_structures!(system::System{T}, pdb_path::String) where {T<:Real}
    structure_list_per_chain = Dict() # TODO können structures über chains hinweg existieren?
    for chain in eachchain(system)
        structure_list_per_chain[chain.name] = []
    end
    for line in readlines(pdb_path)
